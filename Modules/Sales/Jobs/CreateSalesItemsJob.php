...
class CreateSalesItemsJob implements ShouldQueue
{
    ...
    public function createKit(Invoice $invoice,Item $dbItem,$itemRequestData,$index)
    {
        ...
        $data['qty'] = $qty;
        $data['total'] = $dbItem->moneyFormatter($data['price'] * $data['qty']);
        ...
        $this->createKitItems($itemPureCollection,$dbItem,$invoice,$invoiceKitItem,$qty,$index);
        dispatch(new UpdateKitDataDependingOnItemsJob($invoiceKitItem));

    }

    public function createKitItems($itemPureCollection,Item $dbKit,Invoice $invoice,InvoiceItems $baseItem,$qty,$index)
    {

        foreach ($dbKit->items as $child) {
            $item = $child->item;
            ...
            $sendData['qty'] = (int)$child['qty'] * $qty;
            $sendData['discount'] =$dbKit->moneyFormatter($child['discount'] * $qty);
            $sendData['price'] = $dbKit->moneyFormatter((int)$child['price']);
            $sendData['belong_to_kit'] = true;
            $sendData['kit_id'] = $dbKit->id;
    ...
    public function createItem($item,$itemRequestData,$index)
    {
        ...
        if(!$item->isService() && !$this->invoice->isPending())
        {
            ...
            if($item->isNeedSerial())
                dispatch(new ChangeItemSerialsStatusJob($item,$itemRequestData['serials'],$this->availableSerialsStatus,'saled'));
    ...
    public function addPureItem(Invoice $invoice,Item $dbItem,$itemRequestData,$index = 0)
    {
        ...
        if (!$showInPrint)
            $invoice->update(['printable_price' => false]);
        $data['printable'] = $showInPrint;
        $data['parent_kit_id'] = $itemPureCollection->get('belong_to_kit') ? (int)$itemPureCollection->get('kit_id') : 0;
