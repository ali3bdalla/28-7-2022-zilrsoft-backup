<template>
  <div class="container">
    <div
      class="grid  p-2 grid-cols-2 bg-gray-200 md:grid-cols-3 lg:grid-cols-5 gap-1 lg:gap-2 my-5"
    >
      <a
    ...
    <div class="mt-10">
      ...
      <div
        class="grid p-2 grid-cols-2 bg-gray-200 md:grid-cols-4  gap-1 lg:gap-2 mb-5 mt-3"
      >
        <a
          :href="`/web/products/${item.id}`"
          v-for="item in items"
          :key="item.id"
        >
          <ProductListItemComponent :item="item"></ProductListItemComponent>
        </a>
      </div>
    </div>
  </div>
