  ...
  <div class="cart__totals-buttons">
    <div>
      <div class="cart__total-amount">
        <span>{{ $page.$t.cart.total }} (<span class="inc_tax">{{
            $page.$t.cart.inc_vat
          }} 15%</span>) </span>
        <div>
      ...
      <div v-show="activePage === 'select_shipping_method' && shippingMethodId">
        <div class="cart__total-amount">
          ...
          <div>
            <display-money :money="getTotalShippingWeight()"></display-money> {{  $page.$t.products.kg }}
          </div>
        </div>
        <div class="cart__total-amount">
          <span>{{ $page.$t.cart.shipping_total }} </span>

          <div v-if="getTotalShippingSubtotal() > 0">
            <display-money :money="getTotalShippingSubtotal()"></display-money>
          </div>
        ...
        <div class="cart__total-amount">
          ...
          <div>
            <display-money :money="getOrderNetAmount()"></display-money> {{  $page.$t.products.sar }}
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-else>
      <div v-if="$page.client_logged">
        <div>
          ...
          <button
            class="text-center flex items-center justify-center mt-2 text-gray-900 text-sm mt-2 w-full"
            v-if="activePage === 'checkout'"
      ...
      <a
        ...
        class="proceed-btn"
        href="/web/sign_in"
      >{{$page.$t.cart.login_to_checkout}}</a>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getTotalShippingWeight() {
      ...
      for (let index = 0; index < items.length; index++) {
        if (items[index].weight) {
          totalWeight +=
            parseFloat(items[index].weight).toFixed(2) *
    ...
    getTotalShippingAmount() {
      ...
      const maxBaseWeight = parseFloat(
        this.getShippingMethod().max_base_weight
      );
