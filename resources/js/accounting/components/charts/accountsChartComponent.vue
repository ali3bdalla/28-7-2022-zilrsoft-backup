<template>
  ...
  >
    <li class item="account" v-for="(account, index)  in accountslist" :key="account.id">
      ...
      >{{ account.locale_name}}&nbsp;&nbsp;</span>
      <a :href="'/accounts/create?parent_id=' + account.id" class="btn btn-outline-primary">
        <i class="fa fa-plus-circle"></i>
      </a>
      <a :href="'/accounts/' + account.id + '/edit'" class="btn btn-outline-primary">
        <i class="fa fa-edit"></i>
      </a>
      <a @click="confirmDelete(account)" class="btn btn-outline-primary">
...
export default {
  ...
  methods: {
    ...
    confirmDelete(account) {
      ...
      this.$dialog
        .confirm(
          "هل انت متاكد من هذه العملية ؟ ستفقد الحسابات الفرعية لهذا الحساب ، ستكون قادراً على عرض العمليات السابقة  التي تمت على هذا الحساب والحسابات الفرعية منه ، ننصحك بالغاء هذه العملية اذا كنت لا تفهم ماذا تفعل",
          options
        )
        .then((dialog) => {
