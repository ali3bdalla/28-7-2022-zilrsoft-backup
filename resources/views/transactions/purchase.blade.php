    ...
    <tr style="border:none">
        ...
        @if($index==0)
            ...
            <th width="20%" style="vertical-align: inherit;" rowspan="{{ count
                                ($invoice_transactions) }}"><a href="{{route('management.purchases.show',$purchase->id)
                                }}">{{$purchase->invoice->title}}</a></th>
            <th width="10%" style="vertical-align: inherit;"
                rowspan="{{ count($invoice_transactions) }}"><a href="{{ route('management.transactions.show',$transaction->id)
                                    }}">{{
